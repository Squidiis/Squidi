...
class level_system_control_button(discord.ui.View):
    ...
    @discord.ui.button(label="Yes", style=discord.ButtonStyle.gray, custom_id="yes_button_control")
    async def yes_button_callback_control(self, button, interaction):

        guild_id = str(interaction.guild.id)

        if interaction.custom_id == "yes_button_control":

            if interaction.user.guild_permissions.administrator:

                try: 

                    setup_overwritten_status = ""
...
class level_roles_buttons_role(discord.ui.View):
    ...
    async def yes_button_callback(self, button, interaction):
        ...
        set_level_roles_connect = level_connector.levelconnect()
        my_cursor = set_level_roles_connect.cursor()
        ...
        if interaction.custom_id == "yes_button_role":
            ...
            if interaction.user.guild_permissions.administrator:
                ...
                try:
                    ...
                    set_level_roles_connect.commit()
                    ...
                    if set_level_roles_connect.is_connected():
                    
                        my_cursor.close()
                        set_level_roles_connect.close()
...
class levelup_channel_buttons(discord.ui.View):
    ...
    async def yes_button_callback_levelup(self, button, interaction):
        ...
        levelup_button_connect = level_connector.levelconnect()
        ...
        if interaction.custom_id == "Yes_button_levelup_channel":
            ...
            if interaction.user.guild_permissions.administrator:
                ...
                try:
                    ...
                    if levelup_button_connect.is_connected():
                    
                        my_cursor.close()
                        levelup_button_connect.close()
...
class reset_level_button(discord.ui.View):
    ...
    async def yes_button_callback_levelreset(self, button, interaction):
        ...
        if interaction.user.guild_permissions.administrator:
            ...
            reset_button_connect = level_connector.levelconnect()
            ...
            if interaction.user.id == global_author_id_resetlevel:
                ...
                try:
                    ...
                    if reset_button_connect.is_connected:

                        reset_button_connect.close()
                        my_cursor.close()
...
class level_system_check():
    ...
    def blacklist_check(self, guild_id, guild_check):

        if isinstance(guild_check.channel, discord.TextChannel):

            levelsystem_blacklist_connect = level_connector.levelconnect()
            ...
            if guild_check.channel.category is not None:
                                    
                if str(guild_check.channel.category.id) in blacklist[1]:
                    return True

                if str(guild_check.channel.id) in blacklist[0]:
                    return True
                ...
                for blacklisted_roles in blacklist[2]:
                    if blacklisted_roles == None:
                        ...
                        blacklist_role = guild_check.guild.get_role(int(blacklisted_roles))
                        if blacklist_role in guild_check.author.roles:
                            return True
...
class level_system(commands.Cog):
    ...
    async def on_guild_remove(self, guild):
        ...
        setup_level = level_connector.levelconnect()
    ...
    async def on_message(self ,message):
        ...
        connection_to_db_level = level_connector.levelconnect()
        ...
        if all_infos_setup:
            ...
            if "off" in all_infos_setup[1] and guild_ID_insert in all_infos_setup[0]:
                ...
                # Blacklist check
                check_blacklist = level_system_check.blacklist_check(self, guild_id=guild_ID_insert, guild_check=message)
                            
                if isinstance(message.channel, discord.TextChannel):
                    
                    # Variables that enter the data of new users
                    sql_insert_query_level = "INSERT INTO levelsystem_info (guild_ID, user_ID, user_level, user_XP, rank_card, user_name, voice_time) VALUES (%s, %s, %s, %s, %s, %s, %s)"        
                    level_values = [guild_ID_insert ,user_ID_insert ,user_level_insert ,user_xp_insert ,rank_card_insert ,user_name_insert, voice_time_insert]
                                    ...
                                    if xp_from_user >= xp_need_next_level:
                                        ...
                                        try:
                                            ...
                                            if None not in all_levelup_channels:
                                                ...
                                                if connection_to_db_level.is_connected():

                                                    my_cursor.close()
                                                    connection_to_db_level.close()
    ...
    async def reset_levels_slash(self, ctx):
        ...
        realybutton = Button(label="Yes", style=discord.ButtonStyle.blurple, custom_id="Are_you_sure")
        notsurebutton = Button(label="No", style=discord.ButtonStyle.blurple, custom_id="Not_sure")

        view=View()

        view.add_item(realybutton)
        view.add_item(notsurebutton)
    ...
    async def leaderboard(self, ctx):
        ...
        leaderboard_connect = level_connector.levelconnect()
    ...
    async def level_system_control(self, ctx):
        ...
        level_control_button_yes = Button(label="Activate ", style=discord.ButtonStyle.blurple, custom_id="yes_level_control")
        level_control_button_no = Button(label="Deactivate", style=discord.ButtonStyle.blurple, custom_id="no_level_control")

        view = View()

        view.add_item(level_control_button_yes)
        view.add_item(level_control_button_no)

        guild_Id_control = str(ctx.guild.id)

        status_level_first, guild_level_control_first = [], []
        
        my_conrtol_level_db = level_connector.levelconnect()
        my_curser_control = my_conrtol_level_db.cursor()
        ...
        all_infos_setup = my_curser_control.fetchall()

        active_deactive = "0"

        if all_infos_setup:
     
            if all_infos_setup[1] == "on":
                active_deactive = "Activated"
            elif all_infos_setup[1] == "off":
            ...
            try:
                ...
                if my_conrtol_level_db.is_connected:

                    my_conrtol_level_db.close()
                    my_curser_control.close()
        ...
        async def button_callback(interaction: discord.InteractionResponse):
            ...
            if interaction.custom_id == "yes_level_control":

                level_control_setup = "on"
                
                if status_level_first == "on" and guild_Id_control in  guild_level_control_first:
                    ...
                    try: 
                        ...
                        yes_level_control_values = (level_control_setup, guild_Id_control)
                        my_curser_control.execute(yes_level_control, yes_level_control_values)
            ...
            if interaction.custom_id == "no_level_control":
    
                level_control_setup = "off"
                
                if status_level_first == "off" and guild_Id_control in  guild_level_control_first:
                    ...
                    try: 
                        ...
                        no_level_control_values = (level_control_setup, guild_Id_control)

                        my_curser_control.execute(no_level_control, no_level_control_values)
        ...
        level_control_button_yes.callback = button_callback
        level_control_button_no.callback = button_callback 
    ...
    async def set_textchanels_level_blacklist(self, ctx, channel: Option(discord.TextChannel, description="Select a text channel you want to exclude from the level system")):
        ...
        level_connection_blacklist = level_connector.levelconnect()
        ...
        if blacklist_items:
            ...
            try:
                ...
                if level_connection_blacklist.is_connected:

                    level_connection_blacklist.close()
                    my_cursor.close()
    ...
    async def remove_channel_blacklit(self, ctx, channel:Option(discord.TextChannel, 
        ...
        remove_channel_blacklist = level_connector.levelconnect()
        ...
        if check_blacklist_result:
            ...
            try:
                ...
                if remove_channel_blacklist.is_connected:

                    remove_channel_blacklist.close()
                    my_cursor.close()
    ...
    async def add_category_blacklist(self, ctx, category:Option(discord.CategoryChannel, description="Add a category to the blacklist!")):
        ...
        blacklist_connecter = level_connector.levelconnect()
        my_cursor = blacklist_connecter.cursor()
        ...
        if all_blacklist_items:
            ...
            try:
                ...
                if blacklist_connecter.is_connected:

                    blacklist_connecter.close()
                    my_cursor.close()
    ...
    async def add_role_blacklist(self, ctx, role:Option(discord.Role, description="Select a role to be added to the blacklist!")):
        ...
        blacklist_connect = level_connector.levelconnect()
        ...
        if blacklist_items:
            ...
            try:
                ...
                if blacklist_connect.is_connected:

                    blacklist_connect.close()
                    my_cursor.close()
    ...
    async def remove_role_blacklist(self, ctx, role:Option(discord.Role, description="Select a role you want to remove from the blacklist!")):
        ...
        connect_blacklist = level_connector.levelconnect()
        ...
        if all_blacklist_items:
            ...
            try:
                ...
                if connect_blacklist.is_connected():

                    connect_blacklist.close()
                    my_cursor.close()
    ...
    async def set_level_role(self, ctx, role:Option(discord.Role, description = "Select a role that you want to assign from a certain level onwards"),
        ...
        set_level_role_connect = level_connector.levelconnect()
        ...
        if all_level_role_items == None:
            ...
            if level <= 999:
                ...
                try: 
                    ...
                    if set_level_role_connect.is_connected:

                        set_level_role_connect.close()
                        my_cursor.close()
    ...
    async def remove_level_role(self, ctx, role:Option(discord.Role, description="Select a level role that you want to remove")):
        ...
        remove_level_role_connect = level_connector.levelconnect()
        ...
        if remove_level_role:
            ...
            try:
                ...
                if remove_level_role_connect.is_connected:

                    remove_level_role_connect.close()
                    my_cursor.close()
    ...
    async def show_all_level_roles(self, ctx):
        ...
        show_level_role_connect = level_connector.levelconnect()
        ...
        if all_items_level_role:
            ...
            if show_level_role_connect.is_connected:
                
                show_level_role_connect.close()
                my_cursor.close()
    ...
    async def set_levelup_channel(self, ctx, channel:Option(discord.TextChannel, description="Select a channel in which the level up message should be sent")):
        ...
        levelup_channel_connect = level_connector.levelconnect()
        ...
        if None in levelup_infos:
            ...
            try:
                ...
                if levelup_channel_connect.is_connected:
                                
                    levelup_channel_connect.close()
                    my_cursor.close()
    ...
    async def disable_levelup_channel(self, ctx):
        ...
        levelup_connect = level_connector.levelconnect()
        ...
        if res == False:
            ...
            try: 
                ...
                if levelup_connect.is_connected:
                                    
                    levelup_connect.close()
                    my_cursor.close()
    ...
    async def show_levelup_channel(self, ctx):
        ...
        levelsystem_connect = level_connector.levelconnect()
        ...
        if levelsystem_connect.is_connected:

            levelsystem_connect.close()
            my_cursor.close()
